...
| GET | `/me/notifications` | ✅ | **Inbox** (bonus-expiry reminders). `?unreadOnly=true&limit=&offset=`. Newest-first. |
| POST | `/me/notifications/read` | ✅ | Mark my notifications read. Body `{ ids: string[] }` — only the caller's own rows are touched. |
| POST | `/me/fcm-token` | ✅ | Register/refresh my FCM push token. Body `{ token }` — latest wins; stored on the user, never returned. |
...
| GET | `/payments/wallet/allocate` | ✅ | **Read-only** payment-source split for an item price: `?itemPriceInr=99` → named per-bucket amounts `bonus` / `deposits` / `earnings` + `totalCovered` / `remaining` + available `wallet` (bonus first — 10% cap — then deposits → earnings, mirrors the buy flow). |
