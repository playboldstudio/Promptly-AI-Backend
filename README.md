...
| GET | `/payments/wallet/allocate` | ✅ | **Read-only** payment-source split for an item price: `?itemPriceInr=99` → named per-bucket amounts `bonus` / `deposits` / `earnings` + `totalCovered` / `remaining` (deposits → earnings → bonus, 10% bonus cap). |
